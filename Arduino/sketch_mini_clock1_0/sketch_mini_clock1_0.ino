...
//byte clock_mode = 4;                     // Default clock mode. Default = 4 (setup_menu) - Marcelo
...
Button buttonC = Button(4, BUTTON_PULLUP);      // Setup button C (using button library) - Marcelo
...
void plot (byte x, byte y, byte val) {
  ...
  x = x +1;  //Deslocar uma fileira de leds para a esquerda. Marcelo
...
void small_mode() {
  ...
  while (run_mode()) {
    ...
    //if (buttonC.uniquePress()) {  // Marcelo
      switch_mode();
      return;
    }
    ...
    if (buttonC.uniquePress()) {
        clock_mode = 4;                     // Default clock mode. Default = 4 (setup_menu) - Marcelo
    }
...
void switch_mode() {
  ...
  for (int count = 0; count < 35 ; count++) {
    ...
    //if (buttonC.uniquePress() || firstrun == 1) {    // Marcelo
...
void setup_menu() {
  ...
  byte setting_mode = 0;
  //byte setting_mode = 2;  // set_time - Marcelo
  ...
  for(int count=0; count < 35 ; count++) {
    ...
    if(buttonC.uniquePress() || firstrun == 1){   // Marcelo
      setting_mode = 2;  // set_time - Marcelo
    }
...
void set_intensity() {
  ...
  //while (!buttonC.uniquePress()){   // Marcelo  
